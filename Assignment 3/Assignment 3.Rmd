...
Below I find the naive implied effect of get-out-the-vote interventions on turnout by finding the average difference in turnout between those who were and were not successfully treated. The resulting estimate, importantly, does not account for any of the complications enumerated above, except I drop those subjects who attrited to remove their missing values from calculations.
...
The mean value of turnout (rounded to the nearest thousandth) for people who did and did not receive the treatment are .594 and .474, respectively, and the implied effect of the treatment is the difference between these two average, .119. This can be naitvley interpreted as the treatment, a get-out-the-vote intervention, having caused an 11.9% increase in turnout. 

This number is likely an overestimate of the average effect of the treatment. Two of the three complications to the experiment--non-random assignment and presence of noncompliance--create baseline differences between our treatment and control groups, which biases our estimate toward overstating the effect of get-out-the-vote interventions on turnout. The third complication--attrition--does not create bias in either direction. 

The first complication--non-random assignment--makes living in an urban area a confounder because residential location influences turnout and likelihood of treatment above and beyond random assignment. I verify this below by showing that urbanites are, on average, less likely to vote and are also disproportionately assigned to the control group. (And therefore less likely to be successfully treated.) The combination of these two factors results in a positive bias that overstates the effect of the treatment. 
...
Urbanites and non-urbanites represent roughly half of the sample, yet they both
groups not equally likely to receive treatment. Of the subjects lived in an urban area, .15 and .84 were
successfully and unsuccesfully treated, respectively, while the same subjects living outside an
urban area were .62 and .38, respectively. It is clear that urbanites are substantially
underrepresented in the group that was successfully treated. 

Now I show that this non-random assignment creates baseline differences between those were and were not 
successfully treated because urbanites are, on average, less likely to vote. I do this by comparing 
voting patterns of urbanites and non-urbanites in the control group. 
...
Urban residents in the control group were more less likely to turn out than non-urban residents. The
average difference in turnout was -.0611, or urbanites in the control were 6.11% less likely to turnout.
...
urban residence is a confounder that creates positive bias, which results in the implied effect of GOTV
interventions being overestimated.
...
an overestimation because noncompliers are (by definition) overrepresented among those did not receive
the treatment and also systematically less likely to vote than those those who successfully received the treatment. 
...
efforts. While attrition likely does not contribute to bias, the other two complications, non-random
...
This procedure controls for the bias introduced by the confounder becuase including an urban variable 
...
holding the urban variable constant. Conceptually, when controlling for urban residence, 
the estimated effect of the treatment becomes a weighted average of the relationship between treatment
and outcome of interest for urbanites and non-urbanites.

My estimate decreased by from .119 to .104, for a decrease of .015. This likely occurred because urban
residence makes an individual both less likely to turnout and be successfully treated, and the original
(biased) estimate inaccurately included some of this influence within the implied effect of treatment on
turnout. Making this effect explicit, therefore, reduces the reported relationship between treatment and
turnout.
...
the CATE. This implements the "Wald Estimator" for both urban and non-urban groups. All of the required
assumptions are met--exogeneity because assignment was random, the exclusion restriction because
assignment to treatment likely only influences turnout via the treatment, and presence of compliers
because many subjects were successfully treated. 
...
The numbers above imply that the proportion of compliers for urban and non-urban areas are .753 and .777, respectively. Now we implement the Wald Estimator for both groups to find an unbiased estimate of their CATEs. 
...
I would be assuming that attrition was random. 

This is not always the case. If attrition was systematically related to assignment in some unobserved
way, then dropping the former from the experiment might bias the calculated estimates of the treatment's
effect. As I argued above, I do not think there is reason to worry; not knowing whether someone voted is
probably a result of idiosyncratic factors like moving out of state. 

But just in case, I will estimate the treatment effect assuming that none of the subject who attrited
...
not have voted and in treatment are assumed to have. These assumptions the worst and best case
would decrease and increase the averge difference in turnout between control and treatment,
respectively. 
...
For each group, I follow the same procedure as in part c--calculating and taking a weighted average of
separate CATEs for urban and non-urban areas to account for non-random assignment--but with attrited
observations assumed to have voted or not voted in each scenario. I report the resulting CATEs and then
interpret their meaning as upper and lower bounds on the bias introduced by attrition. I start with the
base case. 
...
estimate I found in part c that did not make any assumptions about the turnout of attrited subjects. 
This result does not help us get a sense of the extent of bias that could occur from attrition. I now 
estimate the lower bound of negative bias by finding the CATE for the worst case scenario. 
